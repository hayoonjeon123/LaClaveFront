...
const MOCK_DELIVERIES = Array.from({ length: 10 }).map((_, i) => ({
    id: i + 1,
    ...
    return (
        <div className="w-full max-w-[1200px] mx-auto space-y-8">
            ...
            <div className="space-y-2">
                <h2 className="text-[30px] font-bold text-gray-800">배송 현황</h2>
                {/* 배송 현황 Stats */}
                <div className="w-full bg-[#E5E5E5] p-6 rounded-lg">
                    <div className="grid grid-cols-3 gap-4 px-8">
                        {/* Stat Card 1 */}
                        <div className="bg-white rounded-[5px] border border-[#5C4033] p-2 h-[100px] flex flex-col justify-between relative">
                            <span className="font-medium text-base text-black pl-2">배송대기</span>
                            <span className="text-3xl font-medium text-gray-900 pl-2">6</span>
                        </div>
                        {/* Stat Card 2 */}
                        <div className="bg-white rounded-[5px] border border-[#5C4033] p-2 h-[100px] flex flex-col justify-between relative">
                            <span className="font-medium text-base text-black pl-2">배송중</span>
                            <span className="text-3xl font-medium text-gray-900 pl-2">4</span>
                        </div>
                        {/* Stat Card 3 */}
                        <div className="bg-white rounded-[5px] border border-[#5C4033] p-2 h-[100px] flex flex-col justify-between relative">
                            <span className="font-medium text-base text-black pl-2">배송 완료</span>
                            <span className="text-3xl font-medium text-gray-900 pl-2">2</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="w-full border border-gray-300 rounded-lg overflow-hidden">
                ...
                <div className="flex border-b border-gray-300">
                    <div className="w-32 bg-[#F9F9F9] flex items-center pl-6 py-2 font-bold text-gray-700">배송 상태</div>
                    <div className="flex-1 bg-white flex items-center px-4 py-2 gap-2">
                        {['배송대기', '배송중', '배송완료'].map((status) => (
                            <button key={status} className="cursor-pointer px-5 py-1.5 border-[2px] border-[#A8A9AD] rounded-full text-sm hover:bg-gray-50 focus:bg-gray-100 transition-colors">
                                {status}
                            </button>
                        ))}
                    </div>
            ...
            <div className="w-full bg-white rounded-lg shadow border border-gray-200 overflow-hidden">
                ...
                <div className="divide-y divide-gray-200">
                    {MOCK_DELIVERIES.map((delivery) => (
                        <div key={delivery.id} className="flex items-center text-center py-4 text-gray-700 font-medium hover:bg-gray-50 text-sm">
                            <div className="w-16">{delivery.id}</div>
                            <div className="w-24">{delivery.orderDate}</div>
                            <div className="w-36">{delivery.orderNumber}</div>
                            <div className="w-20">{delivery.orderer}</div>
                            <div className="flex-1 text-center">{delivery.productName}</div>
                            <div className="w-24">{delivery.purchaseAmount}</div>
                            <div className="w-24">{delivery.actualAmount}</div>
                            <div className="w-24">{delivery.trackingNumber}</div>
                            <div className="w-20">{delivery.paymentMethod}</div>
                            <div className="w-24 flex justify-center">
                                <Select defaultValue={delivery.paymentStatus}>
                            ...
                            <div className="w-24 flex justify-center">
                                <Select defaultValue={delivery.deliveryStatus}>
                                    <SelectTrigger className="cursor-pointer !w-[80px] !h-[30px] !p-0 bg-[#5C4033] text-white !text-[11px] font-medium rounded-[5px] !border-[#A8A9AD] !border-[2px] flex justify-center items-center focus:ring-0 focus:ring-offset-0 [&>svg]:hidden">
                                        <SelectValue />
                                    </SelectTrigger>
                                    <SelectContent>
