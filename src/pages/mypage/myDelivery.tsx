        ...
        <div className="pb-10">
            ...
            <div className="max-w-[700px] mx-auto px-6 pt-6 flex items-center relative mb-4">
                ...
                <div className="flex-1 text-center">
                    <h2 className="text-[24px] font-bold text-[#5C4033] tracking-tight">배송조회</h2>
                </div>
            </div>
            ...
            <div className="max-w-[700px] mx-auto px-6">
                    ...
                    <div className="px-5">
                        ...
                        <div className="flex justify-between px-1">
                            {["발송", "집하", "배송중", "도착"].map((step, idx) => (
                                <span key={idx} className="text-[14px] font-bold text-[#5C4033]">
                                    {step}
                                </span>
                            ))}
                        </div>
                ...
                <div className="py-4 flex items-center gap-5">
                    ...
                    <div>
                        ...
                        <p className="text-[14px] text-[#A8A9AD] font-medium">
                            블랙/FREE
                        </p>
                    </div>
                </div>
                        ...
                        {deliveryLogs.map((log) => (
                            <div key={log.id} className="flex items-start gap-6">
                                <div className="w-[100px] flex-shrink-0">
                                    <p className={`text-[14px] font-bold ${log.active ? "text-black" : "text-[#A8A9AD]"}`}>
                                        {log.date}
                                    </p>
                                    <p className={`text-[13px] font-bold ${log.active ? "text-black" : "text-[#A8A9AD]"}`}>
                                        {log.time}
                                    </p>
                                </div>
                                <div className="flex-1">
                                    <p className={`text-[15px] font-bold mb-0.5 ${log.active ? "text-black" : "text-[#A8A9AD]"}`}>
                                        {log.status}
                                    </p>
                                    <p className={`text-[14px] font-medium ${log.active ? "text-gray-600" : "text-[#A8A9AD]"}`}>
                                        {log.location}
                                    </p>
                                </div>
                            </div>
